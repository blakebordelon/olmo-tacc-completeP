"""
Pre-training script for a small OLMo-3 model (~85M params).

Architecture: d_model=768, 12 heads, head_dim=64, 6 layers (based on olmo2_190M with n_layers=6).
Sequence length: 1024. Global batch size: 1024 * 512 tokens (~524K tokens/step).
"""
...
GLOBAL_BATCH_SIZE = 1024 * 512  # ~524K tokens
LR = 3e-4
...
def build_config(opts: argparse.Namespace, overrides: List[str]) -> ExperimentConfig:
    ...
    # ~85M parameter model: d_model=768, 12 heads, head_dim=64, 6 layers.
    ...
    trainer_config = (
        TrainerConfig(
            ...
            max_duration=Duration.tokens(int(1e12)),  # 1T tokens
        )
